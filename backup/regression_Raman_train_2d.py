...
def conv_model(input_shape, output_shape, dropout_rate, learning_rate):
    ...
    # layer 1  [None, rows, cols, channels]
    model.add(keras.layers.Conv2D(filters=8, kernel_size=(1, 5), padding='same', input_shape=input_shape)) 
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.ReLU())
    model.add(keras.layers.AveragePooling2D(pool_size=(1, 2), strides=2))
    
    # layer 2
    model.add(keras.layers.Conv2D(filters=16, kernel_size=(1, 5), padding='same')) 
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.ReLU())
    model.add(keras.layers.AveragePooling2D(pool_size=(1, 2), strides=2))
    
    # layer 3
    model.add(keras.layers.Conv2D(filters=32, kernel_size=(1, 5), padding='same')) 
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.ReLU())
    model.add(keras.layers.AveragePooling2D(pool_size=(1, 2), strides=2))
    
    # layer 4
    model.add(keras.layers.Conv2D(filters=64, kernel_size=(1, 5), padding='same')) 
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.ReLU())
    
    # layer 5
    model.add(keras.layers.Conv2D(filters=128, kernel_size=(1, 5), padding='same')) 
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.ReLU())
...
def main(unused_argv):
    ...
    model.fit(X, Y, batch_size=batch_size, epochs=max_epoch, validation_split=validation_ratio, callbacks=[lrate, checkpointer, tbCallBack])   
